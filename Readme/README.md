...
graph TD
    ...
    classDef backend fill:#8cc84b,stroke:#333,stroke-width:2px,color:black;
    classDef database fill:#00758f,stroke:#333,stroke-width:2px,color:white;
    
    %% Felhasználó
    User((👨‍💻 Felhasználó))

    %% Frontend - View
    subgraph View [View / Nézet - React Frontend]
        UI[Böngésző / Felület]
        Components[App, UserForm, UserTable]
    end

    %% Backend - Controller
    subgraph Controller [Controller / Vezérlő - Express.js]
        Router[API Végpontok / Routes]
        Logic[Üzleti logika & Validáció]
    end

    %% Adatbázis - Model
    subgraph Model [Model / Modell - MySQL]
        DB[(Users Tábla)]
    end

    %% Kapcsolatok
    User -- Interakció (Kattintás, Gépelés) --> UI
    UI -- Állapot (State) --> Components
    Components -- "HTTP Kérések (Axios: GET, POST...)" --> Router
    Router -- Kérés feldolgozása --> Logic
    Logic -- "SQL Parancsok (mysql2)" --> DB
    DB -. "Adatok / Eredmény" .-> Logic
    Logic -. "JSON Válasz (200 OK)" .-> Components
    Components -. "Felület frissítése" .-> UI

    %% Stílusok alkalmazása
    class View,UI,Components frontend;
    class Controller,Router,Logic backend;
    class Model,DB database;
